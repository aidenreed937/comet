...
```
需求分析 → Domain → Data → Provider → UI → Route → L10n → 质量检查
```
...
1. 使用 `EnterPlanMode` 进行需求分析
2. 拆分为可管理的小任务（每个 < 1 小时）
3. 复杂功能分多个 PR 逐步完成
4. 用户确认规划后再开始编码
...
拆分为：
PR1: Domain 层（实体 + 仓库接口 + 验证器）
PR2: Data 层（DTO + 数据源 + 仓库实现）
PR3: Presentation 层（Provider + 页面 + 组件）
PR4: 集成（路由 + 国际化 + 测试）
```
